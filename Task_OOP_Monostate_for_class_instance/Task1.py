# Предположим, у нас есть класс WeatherStation для метеостанции, которая собирает данные с
# нескольких датчиков. Каждый экземпляр датчика можно создать отдельно, но вы все равно
...
#
# Датчики будут измерять температуру, влажность и давление. Ваша задача определить класс
# WeatherStation, у которого
#
...
#
# метод get_current_data, который возвращает кортеж показаний  temperature, humidity и
# pressure.
# Sample Input:
#
# Sample Output:
...
#
#     def __init__(self):
#         self.__dict__ = WeatherStation.__atr_weather_condition
#
...
#         return tuple(self.__atr_weather_condition.values())
...
class WeatherStation:
    ...
    def update_data(self, *args):
        for key, value in enumerate(WeatherStation.__atr_weather_condition):
            self.__dict__[value] = args[key]
