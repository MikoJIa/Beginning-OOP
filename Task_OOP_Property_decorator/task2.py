...
# количество долларов и устанавливает при помощи него новое значение в атрибут экземпляра
# total_cents, при этом значение центов должно сохранятся.
# В случае, если в сеттер передано число, не удовлетворяющее условию, нужно печатать на
...
# количество центов и устанавливает при помощи него новое значение в атрибут экземпляра
# total_cents, при этом значение долларов должно сохранятся.
...
class Money:
    ...
    @dollars.setter
    def dollars(self, num: int):
        if isinstance(num, int) >= 0:
            self.total_cents = num * 100 + self.cents
        else:
            print('Error dollars')
    ...
    @cents.setter
    def cents(self, num: int):
        if isinstance(num, int) and 100 > num >= 0:
            self.total_cents = self.dollars * 100 + num
        else:
            print(f'Error cents')

    def __str__(self):
        return f'Состояние {self.name} составляет {self.dollars} долларов {self.cents} центов'


Bill = Money(101, 99, 'Bill')
print(Bill)  # Ваше состояние составляет 101 долларов 99 центов
print(Bill.dollars, Bill.cents)  # 101 99
print(Bill.total_cents) # 10199
Bill.dollars = 666
